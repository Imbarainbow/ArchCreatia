...
import studio.robotmonkey.archcreatia.blocks.blockEntities.AbstractManaCondensorEntity;
import studio.robotmonkey.archcreatia.blocks.blockEntities.AbstractManaTankEntity;
import studio.robotmonkey.archcreatia.util.DirectionHelper;
import studio.robotmonkey.archcreatia.util.HasMana;

import java.util.HashMap;

public abstract class AbstractManaCondensor extends BlockWithEntity {

    public static final DirectionProperty FACING;

    protected AbstractManaCondensor(Settings settings) {
        super(settings);
    ...
    public void onPlaced(World world, BlockPos pos, BlockState state, LivingEntity placer, ItemStack itemStack) {
        ...
        if (itemStack.hasCustomName()) {
            ...
            if (blockEntity instanceof AbstractManaCondensorEntity) {
                ((AbstractManaCondensorEntity)blockEntity).setCustomName(itemStack.getName());
            }
        }
    ...
    public void updateConnections(World world, BlockPos pos, BlockState state) {
        BlockEntity blockEntity = world.getBlockEntity(pos);
        ((AbstractManaCondensorEntity) blockEntity).fixInputs();
...
//            System.out.println("Found Something with mana");
            if (blockEntity instanceof AbstractManaCondensorEntity) {
                ((AbstractManaCondensorEntity) blockEntity).setOutput((HasMana) world.getBlockEntity(right));
...
//                System.out.println(BlockRotation.CLOCKWISE_90.rotate(state.get(FACING)));
                ...
                ((AbstractManaCondensorEntity) blockEntity).addInput((HasMana) world.getBlockEntity(inputs.get("up")));
            }
        }
...
//            System.out.println("On left");
            if (world.getBlockState(inputs.get("left")).get(FACING) == state.get(FACING)) {
                ...
                ((AbstractManaCondensorEntity) blockEntity).addInput((HasMana) world.getBlockEntity(inputs.get("left")));
            }
        }
...
//            System.out.println("down");
            if (world.getBlockState(inputs.get("down")).get(FACING) == BlockRotation.COUNTERCLOCKWISE_90.rotate(state.get(FACING))) {
                ...
                ((AbstractManaCondensorEntity) blockEntity).addInput((HasMana) world.getBlockEntity(inputs.get("down")));
            }
        }
    }
    ...
    public void onBlockRemoved(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (state.getBlock() != newState.getBlock()) {
            BlockEntity blockEntity = world.getBlockEntity(pos);
            if (blockEntity instanceof AbstractManaCondensorEntity) {
                ItemScatterer.spawn(world, pos, (AbstractManaCondensorEntity)blockEntity);
                ((AbstractManaCondensorEntity)blockEntity).fixInputs();
